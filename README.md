...
除了默认的 `config/variables.ini` 文件外，你还可以创建自定义的变量配置文件，用于不同环境或测试场景：
...
使用 `--vars-config` 命令行参数指定自定义变量配置文件：
...
2. 如果未找到，则在变量配置文件中查找（默认为 `variables.ini` 或通过 `--vars-config` 指定的自定义配置文件）
...
`--vars-config` 参数允许你指定自定义的变量配置文件，替代默认的 `config/variables.ini` 文件。这在以下场景特别有用：

- 针对不同环境（开发、测试、生产）使用不同的变量集
- 针对不同测试场景使用特定的测试数据
- 团队成员使用各自的配置文件而不影响他人
- 在CI/CD流程中使用环境特定的配置

当使用 `--vars-config` 参数时，框架会：

1. 加载指定的变量配置文件
2. 将文件中的变量加载到全局缓存
3. 在处理请求中的占位符时，优先使用全局缓存中的变量，然后再查找配置文件中的变量
...
**config/dev_variables.ini**
```ini
[VARIABLES]
base_url = https://api-dev.example.com
auth_token = Bearer dev-token-123
user_id = 1001
```

**config/test_variables.ini**
```ini
[VARIABLES]
base_url = https://api-test.example.com
auth_token = Bearer test-token-456
user_id = 2001
```

**config/prod_variables.ini**
```ini
[VARIABLES]
base_url = https://api.example.com
auth_token = Bearer prod-token-789
user_id = 3001
```
...
```bash
# 开发环境测试
python run_tests.py --vars-config config/dev_variables.ini

# 测试环境测试
python run_tests.py --vars-config config/test_variables.ini

# 生产环境测试
python run_tests.py --vars-config config/prod_variables.ini
```

这样，同样的测试用例可以在不同环境中使用不同的配置数据执行，无需修改测试用例本身。
...
A: 可以使用变量占位符 `${variable_name}`，这些变量可以来自变量配置文件、命令行参数、前置测试用例的提取结果或全局缓存。

### Q: 如何为不同环境使用不同的测试数据？

A: 可以为每个环境创建单独的变量配置文件（如 `dev_variables.ini`、`test_variables.ini`、`prod_variables.ini`），然后使用 `--vars-config` 参数指定要使用的配置文件。例如：`python run_tests.py --vars-config config/test_variables.ini`。
