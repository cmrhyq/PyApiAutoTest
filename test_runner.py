...
class TestExecutor:
    ...
    def execute_test_case(test_case_data: Dict[str, Any], is_dependency: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            result['success'] = all(ar.success for ar in assertion_results) if isinstance(assertion_results, list) else assertion_results
    ...
    @classmethod
    def execute_test_cases_in_parallel(cls, test_cases: List[Dict[str, Any]], max_workers: int = 4) -> Dict[str, Dict[str, Any]]:
        """并行执行多个测试用例
...
def test_api_from_excel(test_case_data):
    ...
    with allure.step("测试用例数据"):
        allure.attach(json.dumps(test_case_data, indent=2, ensure_ascii=False), 
                     name="Test Case Data", 
            ...
            if extract_vars:
                with allure.step("提取响应变量"):
                    ...
                    allure.attach(json.dumps(extracted, indent=2, ensure_ascii=False), 
                                 ...
                                 attachment_type=allure.attachment_type.JSON)
                    allure.attach(json.dumps({k: v for k, v in cache._cache.items() if not k.startswith('_')}, indent=2, ensure_ascii=False), 
            ...
            with allure.step("执行断言"):
                ...
                if isinstance(assertion_results, list):
                    for i, result in enumerate(assertion_results):
                        ...
                        allure.attach(f"规则: {result.rule}\n预期: {result.expected}\n实际: {result.actual}\n结果: {status}", 
                                     name=f"断言 {i+1}", 
                                     attachment_type=allure.attachment_type.TEXT)
                    ...
                    if not all(result.success for result in assertion_results):
                        ...
                        failure_message = "\n".join([f"断言失败: {result.rule} - 预期: {result.expected}, 实际: {result.actual}" 
